...
public class KuaiQianService {
    ...
    public Map<String,String> generateKqFormData(Integer userId, String rechargeMoney, String rechargeNo){
        ...
        recharge.setRechargeStatus(YLBContants.RECHARGE_STATUS_PROCESSING);
    ...
    public void handlerKQNotify(HttpServletRequest request) {
        ...
        if(flag){
            //验签通过
            result = rechargeService.handlerRechargeNotify(orderId,payResult,payAmount);

        } else {
            //验签失败
            rechargeService.modifyRechargeStatus(orderId,YLBContants.RECHARGE_STATUS_SIGN_FAIL);
            //日志， 把充值的信息记录下来

            result = Result.fail();
            result.setCodeEnum(CodeEnum.RC_RECHAGE_SIGN_FAIL);
        }
    }

    //调用快钱的支付查询接口
    public Result invokeKqOrderQuery(String rechargeNo) {
        ...
        try {
            ...
            //处理response 的json， 根据 payResult 处理订单。
            if( response != null && response.length() > 0 ){
                ...
                if( jsonObject != null){
                    ...
                    if( detail == null ){
                        //此时订单是 充值失败。
                        rechargeService.modifyRechargeStatus(orderId,YLBContants.RECHARGE_STATUS_FAIL);

                    } else {
                        JSONArray detailArray = jsonObject.getJSONArray("orderDetail");
                        //获取数组的一个值
                        for(int i=0;i< detailArray.size();i++){
                            JSONObject orderJsonObject = (JSONObject) detailArray.get(i);
                            //获取payResult payAmount, orderId
                            result = rechargeService.handlerRechargeNotify(
                                    orderJsonObject.getString("orderId"),
                                    orderJsonObject.getString("payResult"),
                                    orderJsonObject.getString("payAmount"));
                        }
                    }
                }
            } else {
